...
export class BlogService {
  ...
  async create(dto: CreateBlogDto): Promise<BlogResponse> {
    ...
    let excerpt;
    try {
      excerpt = await geminiService.translateOne(dto.content.slice(0, 300), {
        context:
          "Write a 1-2 sentence summary/excerpt of this blog post content for a filter-system factory e-commerce platform",
      });
    } catch {
      excerpt = {
  ...
  async findAll(
    ...
    return buildPaginatedResponse(data.map(toBlogResponse), total, {
      page,
      limit,
      skip,
    });
