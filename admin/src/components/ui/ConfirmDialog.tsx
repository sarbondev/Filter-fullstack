  ...
  return (
    <Modal open={open} onClose={onClose} title={title} size="sm">
      ...
      <div className="flex justify-end gap-3 border-t border-slate-200 pt-4">
        <Button variant="secondary" onClick={onClose} disabled={loading}>{cancelText ?? 'Cancel'}</Button>
        <Button variant={variant} onClick={onConfirm} loading={loading}>{confirmText ?? 'Confirm'}</Button>
      </div>
    </Modal>
  );
}
