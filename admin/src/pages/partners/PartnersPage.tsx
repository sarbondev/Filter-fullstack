...
import { z } from 'zod';
import {
  useGetPartnersQuery, useCreatePartnerMutation, useUpdatePartnerMutation, useDeletePartnerMutation,
...
export default function PartnersPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) {
        await updatePartner({ id: editing.id, data: result.data }).unwrap();
      } else {
        await createPartner(result.data).unwrap();
      }
      setModalOpen(false);
      resetForm();
    } catch (err: unknown) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-slate-900">{t('sidebar.partners')}</h1>
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => { if (deleteId) { await deletePartner(deleteId); setDeleteId(null); } }}
